...
export default function Analyzer() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch(`${BASE_URL}/analyzer/upload/`, {
                method: 'POST',
                body: formData,
            });
            if (!response.ok) {
            ...
            setError('Failed to get analysis. Please check the console for details.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    // Helper to get nested data regardless of API response structure
    ...
    return (
        <div className="analyzer-container">
            <header className="analyzer-header">
                ...
                <p>Get instant analysis and rewrites powered by Gemini AI</p>
            </header>

            <section className="input-section">
                <div className="tabs">
                    <button className={inputType === 'text' ? 'active-tab' : 'tab'} onClick={() => setInputType('text')}>
                        Text Input
                    </button>
                    <button className={inputType === 'file' ? 'active-tab' : 'tab'} onClick={() => setInputType('file')}>
                        File Upload
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    {inputType === 'text' ? (
                        ...
                        <div 
                            className={`file-input-container ${isDragging ? 'active' : ''}`}
                        ...
                        >
                            ...
                            {selectedFile ? (
                                ...
                                <p><strong>Drag & Drop</strong> your file here, or click to select</p>
                            )}
                        </div>
                    )}
                 ...
                 <section className="results-container">
                    ...
                    <div className="key-metrics">
                        <AnalysisCard title="Sentiment">
                            <p className="sentiment-value">{get('sentiment')}</p>
                        </AnalysisCard>
                    ...
                    <div className="results-grid" style={{marginTop: '20px'}}>
                        <AnalysisCard title="Emotions">
                            <div className="pill-container">
                                {(get('emotions') ?? []).map(e => <Pill key={e} text={e} />)}
                            </div>
                        </AnalysisCard>
                        <AnalysisCard title="Main Topics">
                             <div className="pill-container">
                                {(get('topics') ?? []).map(t => <Pill key={t} text={t} />)}
                            </div>
                        </AnalysisCard>
                         <AnalysisCard title="Suggested Hashtags">
                            <div className="pill-container">
                                {(get('hashtags') ?? []).map(h => <Pill key={h} text={h} />)}
                            </div>
                        </AnalysisCard>
                        <AnalysisCard title="Suggestions for Improvement">
